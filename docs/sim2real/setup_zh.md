...
|---|---|---|---|---|---|---|
| 1 | 350 × 200 × 13 mm | 铝合金蜂窝板 | 1 | AL6061-T6 (SS) | 阳极黑色 | 标准件 |
| 2 | 见 `base.3mf`（release 附件） | 3D 打印底座 | 1 | PLA | 无 | 加工件 |

另需 4× M6 内六角螺丝（长度视蜂窝板厚度而定，常用 16 mm），用于把底座固定到蜂窝板上。

蜂窝板为标准目录件 — 350 × 200 × 13 mm AL6061-T6 铝合金蜂窝板，阳极黑色，钻有 **M6 螺纹通孔阵列、25 mm 间距**（距边 25 mm，13 × 7 共 91 孔）。任何匹配该孔阵的平板均可替换；蜂窝板本身无需额外机加工。
...
**尺寸参考。** release 附件的 `hardware/hand-jig/` 目录提供 `assembly.pdf`（带标注的装配图）、`assembly.step`（完整 STEP 实体 — 用任意 CAD 查看器即可测量或二次开发）和 `base.3mf`（可打印底座）。如果你不是直接用随附 CAD、而是从零复现，以下是图纸上的关键数据（单位均为 mm）：

| 尺寸项 | 数值 |
|---|---|
| 装配整体高度（底座 + 蜂窝板） | **146.7 mm** |
| 静止时灵巧手后倾角 | **10.0°** |
| 整机质量 | 约 14.3 kg（主要来自蜂窝板） |
| 蜂窝板 | 350 × 200 × 13 mm，AL6061-T6，阳极黑色 |
| 蜂窝板孔阵 | 91 × M6 螺纹通孔，25 mm 间距，13 × 7 阵列 |
| 3D 打印底座外形 | 约 90（宽）× 93（深）× 134（高）mm |
| 底座 → 蜂窝板固定 | 4 × M6 内六角螺丝穿 φ6.60 过孔，φ11.0 ↧ 6.8 沉头（容纳螺丝头）；2 × φ3.2 定位孔 |
| 底座打印配置 | PLA，0.2 mm 层高，3 圈外墙，50% 填充，0.4 mm 喷嘴（Bambu 配置已捆绑在 `base.3mf` 内） |

装配图上的通用工艺要求：未注尺寸按 **GB/T1804-2000-F**（例如 18–50 mm 段 ±0.10，120–250 mm 段 ±0.20），未注表面粗糙度 Ra 3.2；锐边倒钝去毛刺；满足 RoHS 2.0 / REACH。功能上只有 **10° 后倾角** 和 **沉头孔阵** 对相机取景与螺丝平齐压紧是关键，其余仅为复现便利。
