...
`--agent.upload-model False` 表示 checkpoint 只存本地。移除该参数并设置 `WANDB_API_KEY`，最后一次迭代的 checkpoint 还会作为 model artifact 上传到 W&B。无论是否上传，本地 `.pt` 都会在每个 `save_interval` 触发点写入。
...
<Callout type="info">
  ...
  `WujiHand_Reorient_Light` 可在约 12 GB 显存的 GPU 上运行，但收敛到较弱的策略，在较难的翻转上偶尔掉 cube、出现卡手。
</Callout>
...
评测核心可 import，因此可从 Python 直接驱动批量评测，无需走 CLI：
