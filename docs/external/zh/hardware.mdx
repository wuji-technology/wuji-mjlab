...
完成后将得到一台经标定的相机、一个 3D 打印的 ArUco cube、一个手腕 AprilTag 世界坐标系、一台装在治具上的 Wuji Hand，以及按实际装置填好的 `camera.yaml` 与 `cube_tags.json`。
...
上述方式默认都会把文件安装到 `/opt/MVS/` 下。安装完成后应能看到 `/opt/MVS/lib/64/libMvCameraControl.so`（运行时库）、`/opt/MVS/Samples/64/Python/MvImport/`（Python 绑定）、`/opt/MVS/bin/MVS`（设备发现 GUI）。
...
4. 若贴纸无法恰好落在距面中心 18 mm 处，在 `cube_tags.json` 中重新标定 `tag_center_offset`。

在 ArUco 检测层面，贴纸 cube 与双材料 cube 功能等价，但更容易出现贴纸脱落和对齐偏差。条件允许时优先选用 `.3mf`。
