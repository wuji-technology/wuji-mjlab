...
ONNX 策略通过一个 loader 加载，它读取 sidecar JSON 中导出时捕获的控制模式参数（`action_scale`、`ema_alpha`、`ctrl_dt`、`history_len`）。这让部署推理与产出该 ONNX 的仿真策略完全一致。
...
在第二个终端中（`vision` 已运行），确认 cube 位姿正在端口 5555 上发布：
