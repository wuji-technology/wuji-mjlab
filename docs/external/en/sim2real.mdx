...
- Cube drops to lost repeatedly — raise `pnp.reproj_threshold` to ~8.0 px and re-check the camera intrinsics calibration. If axes look swapped, fix `face_rotations` in `cube_tags.json` or re-stick the offending tag.
...
- **Axis mismatches** — rotate the physical cube around one face axis and confirm the rendered cube rotates around the same axis. A mirrored or 90°-off rotation means the `face_rotations` in `cube_tags.json` are wrong, or a tag was placed in the wrong orientation.
...
| Policy diverges on the first step | Tag orientation mismatch | Fix `face_rotations` in `cube_tags.json` or re-place the offending tag |
